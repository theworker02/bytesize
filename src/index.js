...
function formatIec(bytes) {
  return formatWith(bytes, IEC, 1024);
}

function formatSi(bytes) {
  return formatWith(bytes, SI, 1000);
}

function convert(input, to) {
  const bytes = typeof input === "number" || /^[0-9]+$/.test(String(input))
    ? Number(input)
    : parseSize(input);
...
module.exports = { parseSize, formatIec, formatSi, convert };
